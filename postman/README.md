...
4. Run any request. `station_id` defaults to `USFL0316`; change it to any
...
https://build.usclimatedata.com/contact. "Normals daily" requires at least the
Developer tier. A Free-tier key gets a 403 on those three. Each gated request
includes a Postman test that accepts either a successful response or the
documented plan-access response and verifies the `forbidden` error code on 403.
